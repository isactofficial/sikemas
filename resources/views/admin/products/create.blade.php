...
<head>
	...
	<style>
		...
		* , *::before, *::after{ box-sizing:border-box; }
		...
		.thumb{ width:56px; height:44px; border-radius:8px; overflow:hidden; border:1px solid #FFD1B2; background:#FFF6F1; display:flex; align-items:center; justify-content:center; }
		...
		@media(max-width:767px){
			/* match Articles create: main pushed below fixed header, compact padding */
		...
		<form class="skm-form" action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
			...
			<div class="skm-grid-2">
				<div>
					...
					<input class="skm-input" type="number" step="0.01" name="price" id="price" value="{{ old('price') }}">
				</div>
				<!-- Status sengaja disembunyikan; default di controller = active -->
			</div>
			<div>
				<label for="description">Description</label>
				<textarea class="skm-textarea" name="description" id="description" rows="5">{{ old('description') }}</textarea>
			</div>
			<div>
				<label for="notes">Catatan Lain</label>
				<textarea class="skm-textarea" name="notes" id="notes" rows="4">{{ old('notes') }}</textarea>
			</div>
...
<script>
	(function(){
		...
		dz.addEventListener('drop', e => {
			...
			if(file){
				input.files = e.dataTransfer.files; // attach to input for form submit
				showPreview(file);
			}
		});
	})();
