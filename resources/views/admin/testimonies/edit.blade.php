...
<main class="skm-admin-main">
	<div class="skm-card">
		...
		<form class="skm-form" action="{{ route('admin.testimonials.update', $testimony) }}" method="POST" enctype="multipart/form-data">
			@csrf
			@method('PUT')
			<div>
				...
				<div id="thumbs" class="thumbs" aria-live="polite">
					@if($testimony->image)
						<div class="thumb"><img src="{{ asset('storage/'.$testimony->image) }}" alt="Current image"></div>
					@endif
				</div>
			</div>
			<div>
				<label for="name">Name</label>
				<input class="skm-input" type="text" name="name" id="name" value="{{ old('name', $testimony->name) }}" required>
			</div>
			<div>
				<label for="job">Job</label>
				<input class="skm-input" type="text" name="job" id="job" value="{{ old('job', $testimony->job) }}">
			</div>
			<div>
				<label for="testimony">Testimony</label>
				<textarea class="skm-textarea" name="testimony" id="testimony" rows="6">{{ old('testimony', $testimony->testimony) }}</textarea>
			</div>
			<div class="skm-actions">
				<a href="{{ route('admin.testimonials.index') }}" class="skm-btn skm-btn-secondary">Cancel</a>
				<button type="submit" class="skm-btn skm-btn-primary">Update</button>
			</div>
		</form>
	</div>
</main>
